...
def supervised_train_step(
    ...
    color_aug_fn,
    inst_lossfn,
    # class_lossfn, #AZ: no se necesita para nuestro objetivo
    device,
    params,
):
    ...
    gt_inst = gt_saug[:, 0]
    # gt_ct = gt_saug[:, 1] #AZ: no se necesita para nuestro objetivo
    if cpv_3c_model:
        gt_3c = gt_saug[:, 2]
    ...
    with torch.autocast(
        ...
        # gt_ct = center_crop(gt_ct, H, W) #AZ: no se necesita para nuestro objetivo

        pred_inst = out_fast[:, : params["inst_channels"]]
        # pred_class = out_fast[:, params["inst_channels"] :] #AZ: no vamos a predecir clases

        # gt_3c = torch.cat(gt_3c_list, axis=0)
        if cpv_3c_model: #AZ: esta funcion if es nueva
            gt_3c = center_crop(gt_3c, H, W)
            instance_loss = inst_lossfn(pred_inst, gt_inst, gt_3c)

        else:
            instance_loss = inst_lossfn(pred_inst, gt_inst)
        
        loss = instance_loss
        # class_loss = class_lossfn(pred_class, gt_ct.long()) #AZ: no nos interesa
        # loss = (
        #     params["loss_lambda"] * instance_loss
        #     + (1 - params["loss_lambda"]) * class_loss
        # )
    # print("inst_loss: ", instance_loss.item(), "class_loss: ", class_loss.item()) #AZ: codigo original
    print("inst_loss: ", instance_loss.item(), "class_loss: ") #AZ: codigo nuevo
    return loss
