...
def new_gan_model(disc_HE, gen_HE, disc_IHC, gen_IHC, gen_frozen_layers, disc_frozen_layers):
    ...
    for layer in features_genIHC[:gen_frozen_layers]:        # Freeze layers 0 to 4
        for param in layer.parameters():
            param.requires_grad = False

    for layer in features_genIHC[gen_frozen_layers:]:        # Train layers 5 to 5
        for param in layer.parameters():
            param.requires_grad = True
    ...
    for layer in features_discIHC[:disc_frozen_layers]:      # Freeze layers 0 to 2
        for param in layer.parameters():
            param.requires_grad = False

    for layer in features_discIHC[disc_frozen_layers:]:      # Train layers 3 to 3
        for param in layer.parameters():
            param.requires_grad = True
    ...
    # for param1, param2 in zip(disc_HE.model[-1].parameters(), disc_last_layer.parameters()):
    #     print(disc_HE.model[-1])
    #     print(disc_last_layer)
...
def main():
    ...
    finetuned_discHE, finetuned_genHE, finetuned_discIHC, finetuned_genIHC = new_gan_model(disc_HE, gen_HE, disc_IHC, gen_IHC, gen_frozen_layers=5, disc_frozen_layers=3)
