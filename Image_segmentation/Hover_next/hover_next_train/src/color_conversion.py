...
class HED_normalize_torch(torch.nn.Module):
    ...
    def color_norm_hed(self, img):
        ...
        sigmas = torch.empty(B, 3).uniform_(-self.sigma, self.sigma).to(img.device)
        sigmas = self.rng(self.sigma, B)
        biases = torch.empty(B, 3).uniform_(-self.bias, self.bias).to(img.device)
...
def color_augmentations(train, sigma=0.05, bias=0.03, s=0.2, rank=0):
    if train:
        ...
        # )
        data_transforms = HED_normalize_torch(sigma, bias, rank=rank)
