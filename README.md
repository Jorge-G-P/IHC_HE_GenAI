...
4. Change configuration settings and hyperparameters for the CycleGAN model
...
5. Run the following command on the terminal to start training the model:
	```
...
- Place the five weight files in IHC_HE_GenAI/pretrained_models. The folder should end up like this:
	...
	```
´
- Run the following command on the terminal if it is the first time you execute it, to generate preprocessed data.:
	
	```
 	python Datasets/Endonuke/preprocessing.py
  	```
