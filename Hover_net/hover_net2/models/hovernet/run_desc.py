...
def viz_step_output(raw_data, nr_types=None):
    ...
    # Extract the relevant dimensions (height and width)
    img_shape = imgs.shape[1:3]
    true_np_shape = true_np.shape[1:3]
    pred_np_shape = pred_np.shape[1:3]

    # Determine the minimum shape across all dimensions
    aligned_shape = np.min([img_shape, true_np_shape, pred_np_shape], axis=0)
    ...
    for idx in range(imgs.shape[0]):
        ...
        true_viz_list = [img]
        true_viz_list.append(colorize(cropping_center(true_np[idx], aligned_shape), 0, 1))
        true_viz_list.append(colorize(cropping_center(true_hv[idx][..., 0], aligned_shape), -1, 1))
        true_viz_list.append(colorize(cropping_center(true_hv[idx][..., 1], aligned_shape), -1, 1))
        if nr_types is not None:  # TODO: a way to pass through external info
            true_viz_list.append(colorize(cropping_center(true_tp[idx], aligned_shape), 0, nr_types))
        true_viz_list = np.concatenate(true_viz_list, axis=1)

        pred_viz_list = [img]
        pred_viz_list.append(colorize(cropping_center(pred_np[idx], aligned_shape), 0, 1))
        pred_viz_list.append(colorize(cropping_center(pred_hv[idx][..., 0], aligned_shape), -1, 1))
        pred_viz_list.append(colorize(cropping_center(pred_hv[idx][..., 1], aligned_shape), -1, 1))
        if nr_types is not None:
            pred_viz_list.append(colorize(cropping_center(pred_tp[idx], aligned_shape), 0, nr_types))
